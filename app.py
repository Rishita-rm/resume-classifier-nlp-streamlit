...
if menu == "🏠 Home":
    ...
    st.write(
        "Upload your resume to get predicted industry categories using NLP and Machine Learning.")
    st.markdown(
        "This tool helps job seekers and recruiters match resumes to domains using AI.")
...
# ------------------- Upload Resumes ------------------- #
    ...
    uploaded_files = st.file_uploader("Upload your resumes", type=[
    ...
    if uploaded_files:
        ...
        df = pd.DataFrame(resume_data).sort_values(
            "Confidence (%)", ascending=False)
        st.success(f"✅ Processed {len(df)} resume(s)")
        st.dataframe(df, use_container_width=True)

        st.markdown("### 📊 Confidence Comparison")
        fig = px.bar(df, x="Filename", y="Confidence (%)",
                     color="Predicted Category", title="Resume Ranking")
        st.plotly_chart(fig)
...
# ------------------- Compare with JD ------------------- #
    ...
    resume_file = st.file_uploader(
        "Upload a single resume", type=["pdf", "docx", "txt"])
